...
begin
  ...
  document := jsonb_build_object('id', p, 'title', 'Проверка учебного плана', 'program_code', '09.03.04', 'admission_year', 2026,
    ...
    'disciplines', jsonb_build_array(
      ...
      jsonb_build_object('id', 'choice-a', 'name', 'Выбор А', 'semester', 1, 'choice_group', 'one', 'hours', 72, 'credits', 2),
      jsonb_build_object('id', 'choice-b', 'name', 'Выбор Б', 'semester', 1, 'choice_group', 'one', 'hours', 72, 'credits', 2)));
  ...
  perform public.miniapp_learning_roadmap(a, 'set_completed', jsonb_build_object('id', p, 'discipline_id', 'math', 'completed', true));
  ...
  assert result -> 'progress' -> 0 ->> 'note' = 'Только пользователь А';
  result := public.miniapp_learning_roadmap(b, 'plan', jsonb_build_object('id', p));
  assert result -> 'progress' = '[]'::jsonb;
  ...
  failed := false;
  begin
    perform public.miniapp_learning_roadmap(a, 'set_completed', jsonb_build_object('id', p, 'discipline_id', 'not-present', 'completed', true));
  ...
  assert jsonb_array_length(result -> 'disciplines') = 3;
