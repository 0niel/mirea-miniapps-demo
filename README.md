...
`miniapp-svc-learning-roadmap` — каталог учебных планов РТУ МИРЭА, сравнение программ, предметы и нагрузка по семестрам, выбор дисциплин, заметки, цели и напоминания. Выбранная программа, предметы и заметки сохраняются между устройствами. Обязательные блоки выбора и факультативы учитываются отдельно.
...
`miniapp-svc-student-discounts` — предложения с логотипами сервисов, поиском и фильтрами, избранным, условиями получения, ссылками на источники, копированием промокодов, шарингом и напоминаниями. Можно предложить свою скидку, исправить или отозвать предложение, отслеживать его статус и сообщить о неработающих условиях.
...
Используются Supabase CLI 2.116.0 и Deno 2.9.6. Миграции применяются по одному разу в порядке времени; существующие миграции «Искры» повторно запускать не требуется.
...
supabase db query --linked --project-ref ejzybbyjwtzbibrrwrli --file supabase/migrations/20260906090902_refine_learning_plan_choices.sql
